...
// QueryIssuesCreated chains the current query on the "issuesCreated" edge.
func (uq *UserQuery) QueryIssuesCreated() *IssueQuery {
	query := &IssueQuery{config: uq.config}
...
// the "issuesCreated" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withIssuesCreated; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.IssuesCreated = []*Issue{}
		}
		query.Where(predicate.Issue(func(s *sql.Selector) {
			s.Where(sql.InValues(user.IssuesCreatedColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.IssuesCreated = append(node.Edges.IssuesCreated, n)
		}
	}
